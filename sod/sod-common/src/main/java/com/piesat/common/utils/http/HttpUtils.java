...
//        String pwd = "music#2020";
        String userName = "dsmsys";
        String pwd = "Nmic@100200_";
//        String s = arcGet("http://10.40.120.167:8080/arc/archive/productioncode/info", userName, pwd);
        String s1 = mmdGet("http://10.40.17.45:28298/MMD/api/rest/intf/fetch/normal","getDDDDoc", userName, pwd);
        FileWriter fileWriter = null;
        try {
            fileWriter = new FileWriter("D:/a123.txt");//创建文本文件
            fileWriter.write(s1);//写入 \r\n换行
            fileWriter.flush();
            fileWriter.close();
        } catch (IOException e) {
...
//http://10.40.17.54:28298/MMD/api/rest/intf/fetch/normal?userId=admin&pwd=JuFeng2019&interfaceId=getDDDDoc

        String filePath = "D:/a123.txt";
...
//        Map resultList = objects.stream().map(e->JSONObject.parseObject(e.toString()).getString("c_snetship_id")).collect(Collectors.collectingAndThen(Collectors
...
//            return map;
//        }));
//        objects.stream().map(e->JSONObject.parseObject(e.toString())).filter(e->e.getString("c_snetship_id").equals("d31fba9a-7ef7-11ea-979d-00ffea080bb6")).forEach(System.out::println);

        Map dd = objects.stream().map(e-> JSONObject.parseObject(e.toString())).collect(Collectors.collectingAndThen(Collectors
                .groupingBy(e->e.getString("c_snetship_id"), Collectors.counting()), map->{
            map.values().removeIf(size -> size == 1);
            List<String> tempList = new ArrayList<>(map.keySet());
            return map;
        }));
