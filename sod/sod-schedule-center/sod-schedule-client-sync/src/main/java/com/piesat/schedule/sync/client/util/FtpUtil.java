...
import com.piesat.schedule.sync.client.vo.FtpConfig;
...
public class FtpUtil {
     ...
     * 上传文件到ftp服务器
     */
    public boolean ftpUpload(FtpConfig ftpConfig, ResultT<String> resultT) {
...
//        mFTPClient.addProtocolCommandListener(new PrintCommandListener(new PrintWriter(System.out)));
        ...
        try {
            boolean isConnection = ftp.openConnection(mFTPClient, ftpConfig.getUrl(), ftpConfig.getPort(), ftpConfig.getUsername(), ftpConfig.getPassword());
            if (isConnection) {
                boolean isSuccess = ftp.upload(mFTPClient, ftpConfig.getRemotePath(), ftpConfig.getLocalDir() + File.separator + ftpConfig.getFileName(), resultT);
                if (isSuccess) {
                    log.info("文件上传成功！");
                    result = true;
                } else {
     ...
     * 从ftp服务器下载文件到本地
     */
    public boolean ftpDownload(FtpConfig ftpConfig) {
        boolean result = false;
        FTPClient mFTPClient = new FTPClient();
        // 在控制台打印操作过程
        mFTPClient.addProtocolCommandListener(new PrintCommandListener(new PrintWriter(System.out)));
        ...
        try {
            boolean isConnection = ftp.openConnection(mFTPClient, ftpConfig.getUrl(), ftpConfig.getPort(), ftpConfig.getUsername(), ftpConfig.getPassword());
            if (isConnection) {
                boolean isDownloadOk = ftp.downLoad(mFTPClient, ftpConfig.getFileName(), ftpConfig.getDownDir());
                boolean isCreateOk = ftp.createDirectory(ftpConfig.getRemotePath(), mFTPClient);
     ...
     * @throws IOException
     */
    private boolean openConnection(FTPClient mFTPClient, String host, int port, String account, String pwd)
     ...
     * 登出并断开连接
     */
    public void logout(FTPClient mFTPClient) {
     ...
     * 断开连接
     */
    private void disConnection(FTPClient mFTPClient) {
        if (mFTPClient.isConnected()) {
            try {
     ...
     * @throws IOException
     */
    public boolean downLoad(FTPClient mFTPClient, String remotePath, String localDir) throws IOException {
        // 进入被动模式
     ...
     */
    public boolean upload(FTPClient mFTPClient, String remotePath, String localPath, ResultT<String> resultT) throws IOException {
        // 进入被动模式
        mFTPClient.enterLocalPassiveMode();
        // 以二进制进行传输数据
        mFTPClient.setFileType(FTP.BINARY_FILE_TYPE);
        ...
        if (remotePath.contains("/")) {
            ...
            if (!isCreateOk) {
                ...
                resultT.setErrorMessage("目标文件夹创建失败：{}", remotePath);
                return false;
            }
        }
            ...
            if (mFtpFile != null) {
                ...
                if (remoteSize > localFile.length()) {
                    if (!mFTPClient.deleteFile(remoteFilePath)) {
                        ...
                        boolean isUpload = uploadFile(mFTPClient, remoteFilePath, localFile, 0);
                        System.err.println("是否上传成功：" + isUpload);
                    }
                    return true;
                }
                if (!uploadFile(mFTPClient, remoteFilePath, localFile, remoteSize)) {
        ...
        boolean isUpload = uploadFile(mFTPClient, remoteFilePath, localFile, remoteSize);
        System.err.println("是否上传成功：" + isUpload);
        return isUpload;
    }
     ...
     * @throws IOException
     */
    private boolean uploadFile(FTPClient mFTPClient, String remoteFile, File localFile, long remoteSize) throws IOException {
