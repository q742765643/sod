...
public abstract class ExecuteBaseService {
    ...
    private ConcurrentHashMap<String, ExecutorSync> sync = new ConcurrentHashMap<>();
    ...
    public List<Server> findServer(JobInfoEntity jobInfoEntity, ResultT<String> resultT) {
        ...
        boolean isSync = jobInfoEntity.getType().toLowerCase().startsWith("sync");
        if (isSync) {
            application = discoveryClient.getApplication(GrpcConstant.SCHEDULE_CLIENT_SYNC);
        } else {
            application = discoveryClient.getApplication(GrpcConstant.SCHEDULE_CLIENT_SERVER);
        }
    ...
    public void remote(JobInfoEntity jobInfoEntity, Server server, ResultT<String> resultT) {
        ...
        try {
            ...
            boolean isSync = jobInfoEntity.getType().toLowerCase().startsWith("sync");
            if (isSync) {
                ExecutorSync executorSync = sync.get(serviceName);
                if (executorSync == null) {
                    Class<?> target = ExecutorSync.class;
                    Object invoker = new Object();
                    InvocationHandler invocationHandler = new GrpcServiceProxy<>(target, invoker, server);
                    executorSync = (ExecutorSync) Proxy.newProxyInstance(GrpcHthtService.class.getClassLoader(), new Class[]{target}, invocationHandler);
                    sync.put(serviceName, executorSync);
                }
                executorSync.execute(jobInfoEntity);
            } else {
