  ...
  <section class="handleClearDialog">
    ...
    >
      <el-row>
        ...
        <!--saveDirectory-->
        <el-col :span="24">
          <el-form-item label="存储目录" prop="saveDirectory">
            <el-select
              clearable
              v-model.trim="msgFormDialog.saveDirectory"
              placeholder="请选择"
              filterable
              style="width: 100%"
            >
              <el-option
                v-for="dict in saveDirectoryOptions"
                :key="dict.dictValue"
                :label="dict.dictLabel"
                :value="dict.dictValue"
              ></el-option>
        ...
        <el-col :span="24">
          <el-form-item label="方式">
            <el-checkbox-button @change="changeType">{{msgFormDialog.transferType}}</el-checkbox-button>
          </el-form-item>
        </el-col>
        <el-col :span="12" v-if="msgFormDialog.transferType == '远程'">
          <el-form-item label="IP" prop="ftpIp">
            <el-input
              clearable
              v-model.trim="msgFormDialog.ftpIp"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12" v-if="msgFormDialog.transferType == '远程'">
          <el-form-item label="端口" prop="ftpPort">
            <el-input clearable v-model.trim="msgFormDialog.ftpPort"  />
          </el-form-item>
        </el-col>
        <el-col :span="12" v-if="msgFormDialog.transferType == '远程'">
          <el-form-item label="用户名" prop="ftpUser">
            <el-input
              clearable
              v-model.trim="msgFormDialog.ftpUser"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12" v-if="msgFormDialog.transferType == '远程'">
          <el-form-item label="密码" prop="ftpPwd">
            <el-input clearable type="password" v-model.trim="msgFormDialog.ftpPwd"  />
          </el-form-item>
        </el-col>
        <el-col :span="24">
...
export default {
  ...
  async created() {
    ...
    await this.getDicts("backup_storage_directory").then((response) => {
      this.saveDirectoryOptions = response.data;
    });
    ...
    if (this.handleObj.id) {
      await getSynctofile(this.handleObj.id).then((response) => {
  ...
  methods: {
    changeType(){
      this.transferExchang = !this.transferExchang;
      if(this.transferExchang){
        this.msgFormDialog.transferType = "远程"
      }else {
        this.msgFormDialog.transferType = "本地"
      }
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.msgFormDialog.id != undefined) {
            updateSynctofile(this.msgFormDialog).then((response) => {
            ...
            addSynctofile(this.msgFormDialog).then((response) => {
              if (response.code === 200) {
